...
单 partition 内的并行度与 partition 数解绑：**一个 poll 循环**负责拉取与提交，消息按 key 哈希路由到**固定数量的泳道**，同 key 的消息永远落在同一泳道（串行），异 key 跨泳道完全并发；offset 提交采用**最大连续已完成**位置，崩溃恢复从该位置继续，不丢已提交消息。
...
- **同 key 串行、异 key 并发**：key → lane 的映射是确定性的，同一会话/实体消息天然严格有序，无需额外协调；
- **最大连续 offset 提交**：并发乱序完成下，只提交「从起点到第一个空洞」的连续区间，杜绝"offset 已提交但中间消息未处理"；
- **背压**：单 partition 在途数达到上限即 pause 拉取，低于水位恢复，配合有界泳道队列，内存可控。

## 性能

最大卖点是**单 partition 消费吞吐的提升**。对 IO 密集场景（handler 阻塞在 DB / RPC / HTTP），泳道并发几乎线性扩展——等待 IO 时 goroutine 让出 CPU 去处理其他泳道的消息。

实测（handler 阻塞 1ms、2000 条消息、key 均匀分布，Apple Silicon，`go run ./bench` 复现）：
...
- **8 泳道 ≈ 8 倍吞吐，32 泳道 ≈ 30 倍**，近线性扩展；
- L=1 与顺序消费一致（1.0x），验证引擎本身无额外瓶颈；
- 前提：**key 分布均匀**（无热点 key）、handler 为 **IO 密集**（CPU 密集场景受益有限，需实际压测）。
